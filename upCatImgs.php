...
$sql_select = $con->prepare('SELECT link,hide FROM images WHERE link = ?');
$sql_select->bindParam(1, $imgname);
$sql_select->execute();
$match_result = $sql_select->fetch(PDO::FETCH_ASSOC);

if ($match_result) {
    $matchlink = $match_result['link'];
    if ($match_result['hide']==0) {
        $con=null;
